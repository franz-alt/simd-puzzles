...
benchmark::testrun perform_test(std::uniform_int_distribution<> & distribution, std::mt19937 & generator, std::size_t values)
{
	...
	// naive without SIMD
	{
		...
		for (std::size_t i = 0; i < repeats; ++i)
		{
			...
			simd::algorithms::scale_naive(&data[0], data.size(), &result[0], 3.1415, 42);
	...
	// AVX2 with 'fma' instruction
	{
		...
		for (std::size_t i = 0; i < repeats; ++i)
		{
			...
			simd::algorithms::avx2::scale_fma(&data[0], data.size(), &result[0], 3.1415, 42);
		...
		tr.add_result("AVX2 (fma)", sum / static_cast<double>(repeats));
	}
#endif
